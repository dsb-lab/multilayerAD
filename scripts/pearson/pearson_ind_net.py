...
for level in levels:
    ...
    coef = np.load(f'../results_pearson/{level}_pearson_array.npy')
    pval = np.load(f'../results_pearson/{level}_pval_array.npy')
    ...
    for i, name in enumerate(["unweighted", "weighted"]):
        ...
        adj_file = f'../results_pearson/{level}_adj_df_{name}_pearson.pickle'
        net_file = f'../results_pearson/{level}_network_{name}_pearson.json'
