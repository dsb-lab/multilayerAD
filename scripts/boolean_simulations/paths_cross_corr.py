...
def create_cross_corr_net(source, len_period, noise_prob, node_list=None):
    adj = load_network(f'../results_pearson/bignetwork_weighted_pearson.json', node_list=node_list)
    cross_dict = load_pickle(f'../results_pearson/network_paths/{source}/period_{len_period}/cross_corr_noise_{int(noise_prob*100)}.pickle') #guardabamos mean y std
...
for i, source in enumerate(sources):
    G = create_cross_corr_net(source, len_period, noise_prob, node_list=node_sort)
    path_dict[source] = {}
    for j, target in enumerate(targets):
        print(f'{i+1:2d}/{len(sources):2d} {source:<20}  --> {target:>20}', end='\r')
        try:
            path_dict[source][target] = k_shortest_paths(G, source, target, n_paths, weight='weight')
        except:
            path_dict[source][target] = []
save_pickle('../results_pearson/network_paths/top_pathways.pickle', path_dict)