...
for diag in diags:
    print(diag)
    adj = load_network(f'../results_pearson/bignetwork_{diag}_weighted_pearson.json') # upload la adj matrix de la pearson global network
    npts = adj.shape[0]
    ...
    for i, in_node in enumerate(input_nodes):
        ...
        file = f'../results_pearson/network_paths/{in_node}/period_{len_period}/node_path_{diag}_noise_{int(noise_prob * 100)}.pickle'
        create_folders(file)
