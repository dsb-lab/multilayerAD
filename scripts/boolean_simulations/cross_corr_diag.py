...
for k, in_node in enumerate(input_nodes):
    for j, diag in enumerate(diags):
        print(f'{diag:8s}: {j+1:2d}/{len(diags):2d}, Input: {k+1:02d}/{len(input_nodes):02d}', end='\r')
        path_list = load_pickle(f'../results_pearson/network_paths/{in_node}/period_{len_period}/node_path_{diag}_noise_{int(noise_prob*100)}.pickle')
        ...
        file = f'../results_pearson/network_paths/{in_node}/period_{len_period}/cross_corr_{diag}_noise_{int(noise_prob*100)}.pickle'
        create_folders(file)
