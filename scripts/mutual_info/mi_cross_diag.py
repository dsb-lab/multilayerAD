...
for level1, level2 in combos:
    for diag in diags:
        ...
        df1 = pd.read_csv(f'{level1}_{diag}.csv')
        df1.set_index('RID', inplace=True)
        df2 = pd.read_csv(f'{level2}_{diag}.csv')
        ...
        file = f'mi_array_{level1}_{level2}_{diag}.npy'
        print(file)
        print("")
        ...
        file = f"mi_perms_{level1}_{level2}_{diag}.npy"
        print(file)
        print("")
        np.save(file, coef_array)
