...
for deg, level in enumerate(levels):
    ...
    coef_real = np.load(f'../results_mi/{level}_mi_array.npy')
    coef_perm = np.load(f'../results_mi/{level}_mi_perms.npy')
    ...
    for i, name in enumerate(["unweighted", "weighted"]):
        ...
        adj_file = f'../results_mi/{level}_adj_df_{name}_mi.pickle'
        net_file = f'../results_mi/{level}_network_{name}_mi.json'
