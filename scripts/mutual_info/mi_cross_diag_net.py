...
for level1, level2 in combos:
    for diag in diags:
        ...
        coef_real = np.load(f'../results_mi/mi_array_{level1}_{level2}_{diag}.npy')
        coef_perm = np.load(f'../results_mi/mi_perms_{level1}_{level2}_{diag}.npy')
        ...
        adj1 = load_pickle(f'../results_mi/{level1}_{diag}_adj_df_weighted_mi.pickle') #las networks calculadas antes
        adj2 = load_pickle(f'../results_mi/{level2}_{diag}_adj_df_weighted_mi.pickle')
        ...
        for i, name in enumerate(["unweighted", "weighted"]):
            ...
            adj_file = f'../results_mi/{level1}_{level2}_{diag}_adj_df_{name}_mi.pickle'
            net_file = f'../results_mi/{level1}_{level2}_{diag}_network_{name}_mi.json'
